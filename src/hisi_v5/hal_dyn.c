 ...
 * second, one thread at a time). It is the clock the 3DNR ladder and
 * the eight ladders of hal_ladder.c hang off as well. Each engine keeps
 * its own "moved a step" test and its own failure count: three failed
 * writes in a row stop that engine and leave the others running.
 *
...
void hisi_dyn_tick(hisi_state_t *st)
{
    ...
    bool lad_on = hisi_lad_armed(st);
    unsigned iso = 0;
    unsigned long long exposure = 0;
    long long now;

    if (!dyn_on && !nrx_on && !lad_on)
        return;
    now = dyn_now_ns();
    ...
    if (hisi_iso_query(st, &iso, &exposure)) {
        ...
        if (lad_on)
            hisi_lad_on_exposure(st, iso, exposure);
    }
