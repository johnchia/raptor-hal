/*
 * hal_osd.c -- OSD/overlay ops for HiSilicon gen5, over ss_mpi_rgn.
 *
 ...
 * to VENC rather than to VPSS is not a preference: the per-module region
 * budget (ot_defines.h, quoted in v5_rgn.h) gives VENC eight OVERLAYs and
 * VPSS none -- VPSS's eight are OVERLAYEX, a different struct with a
 * different attach -- so the encoder is the only stage that composites the
 * overlay this file draws. It is also the stage that makes the overlay
 ...
 *
 * (ref/divinus/src/hal/hisi/v4_hal.c:398, gen4's file -- divinus has no V5
 * backend, and the shape of the mistake carries) and never sets `.channel`. Every
 ...
 * field for exactly this reason, and it is gen4's file that it asserts on:
 * the two backends' hisi_osd_mpp_chn are the same eight lines, so the port
 * keeps the fix rather than re-deriving it.
 *
 * WHY ATTACH IS DEFERRED
 *
 * A region can only attach to a VENC channel that exists:
 * ss_mpi_rgn_attach_to_chn on an uncreated channel fails, and there is no "attach later"
 ...
 * (OT_ERR_RGN_BUSY is documented as exactly that: "destroy a venc chn
 ...
 * set is decided in a kernel module and written down nowhere; here the
 * vendor's own sample maps the format through with the CLUT ones beside it
 * (see v5_rgn.h, which also says what the field's misleading comment
 * means). rvd renders BGRA8888, so the alpha channel collapses to one bit:
 * anything not fully transparent is drawn, which keeps thin antialiased
 * strokes visible instead of dropping them. The opposite threshold is what
 * makes small text vanish.
 *
 * The alpha *pair* needs no scaling on this generation, which is the one
 * behavioural difference from gen4's file: OT_RGN_OVERLAY_MAX_ALPHA is 255
 * and so is raptor's, where gen4's OVERLAY_CHN_ATTR_S took 0..128 and
 * v4_rgn_alpha() had to round into it.
 *
 * OP COVERAGE
 *
 ...
 * RSS_ERR_NOTSUP. COVER has no VENC entry in the budget table at all,
...
/*
 * The attach descriptor. THE ONE PLACE .chn_id IS SET; see the divinus
 * note at the top of the file.
 *
 * dev_id is 0 because this part has a single VENC device -- the same 0
 * hisi_bind_vpss_venc writes into its own MPP_CHN_S, and for the same
 * reason.
 */
static void hisi_osd_mpp_chn(int grp, v5_mpp_chn *chn)
{
    memset(chn, 0, sizeof(*chn));
    chn->mod_id = V5_MOD_VENC;
    chn->dev_id = 0;
    chn->chn_id = grp;
}
...
static unsigned int hisi_osd_align_dim(int v)
{
    ...
    u = (unsigned int)v;
    u = (u + 1u) & ~1u;
    if (u > V5_RGN_OVERLAY_MAX_DIM)
        u = V5_RGN_OVERLAY_MAX_DIM;
...
static int hisi_osd_align_pos(int v)
{
    if (v < 0)
        v = 0;
    v &= ~1;
    if (v > V5_RGN_OVERLAY_MAX_XY)
        v = V5_RGN_OVERLAY_MAX_XY;

    return v;
}

/* OVERLAY's layer range is [0, OT_RGN_VENC_MAX_OVERLAY_NUM - 1] -- the
 * header says "depends on OVERLAY max num" and that number is 8. rvd hands
 * layer + 1 through show_region and can exceed it. */
...
static void hisi_osd_fill_chn_attr(const hisi_osd_region_t *r, v5_rgn_chn_attr *ca)
{
    memset(ca, 0, sizeof(*ca));

    ca->show = r->show ? 1 : 0;
    ca->type = V5_RGN_TYPE_OVERLAY;
     ...
     * bg_alpha where it is 0. rvd sends 255/0 for text and this generation
     * takes both verbatim -- opaque glyphs on a fully transparent field.
     *
     * global_alpha_en is not mapped onto anything. There is no per-region
     * constant-alpha mode in ot_rgn_overlay_chn_attr to map it onto, and the
     ...
     */
    ca->overlay.fg_alpha = r->fg_alpha;
    ca->overlay.bg_alpha = r->bg_alpha;

    /*
     * qp_info stays zeroed, and that is the useful setting rather than the
     * lazy one: enable false with a relative delta of 0 is the encoder's
     * own QP under the overlay. `dst` 0 is OT_RGN_ATTACH_JPEG_MAIN, which
     * is what a region on an MJPEG channel wants and is ignored on every
     * other codec. gen4's invert-colour block is not missing here -- V5's
     * per-channel overlay attribute does not have one.
     *
     * union_pad is left zero by the memset above and never written: it is
     * the tail of a union whose widest arm is the COVER one, and the driver
     * copies all 64 bytes. See v5_rgn.h.
     */
}

/* Fill the region attr from a tracked region. */
static void hisi_osd_fill_attr(const hisi_osd_region_t *r, v5_rgn_attr *a)
{
    memset(a, 0, sizeof(*a));

    a->type = V5_RGN_TYPE_OVERLAY;
    a->overlay.pixel_format = V5_PIXEL_FORMAT_ARGB_1555;
    ...
    /* clut is the palette for the CLUT2/CLUT4 formats and is ignored for
     * ARGB; the memset above is the whole of its configuration. */
}
 ...
 * live, and the next start's create returns OT_ERR_EXIST (0x8, where
 * gen4's is 4) for a handle it has never used. It is the same problem
 * hal_init's teardown-first solves for SYS and VB, and it cannot be solved
 * there: RGN has no exit call of its own, and a handle is only known once
 * rvd asks for the region that uses it.
 *
 ...
 * destroy-then-retry on OT_ERR_EXIST, which reaches the same state one call
 * later.
 */
static int hisi_osd_create_rgn(hisi_state_t *st, int handle, hisi_osd_region_t *r)
{
    ...
    if (ret && V5_ERR_ID(ret) == V5_ERR_EXIST) {
        st->rgn.fnDestroy((unsigned int)handle);
        ret = st->rgn.fnCreate((unsigned int)handle, &attr);
    }
    if (ret) {
        HAL_LOG_ERR("ss_mpi_rgn_create(%d, %ux%u ARGB1555) failed: 0x%x", handle, r->width,
                    r->height, ret);
        return RSS_ERR_IO;
    }
...
static int hisi_osd_try_attach(hisi_state_t *st, int handle, hisi_osd_region_t *r)
{
    ...
    if (ret) {
        HAL_LOG_WARN("ss_mpi_rgn_attach_to_chn(region %d, VENC %d) failed: 0x%x", handle, r->grp,
                     ret);
        return RSS_ERR_IO;
    }
    ...
    HAL_LOG_DBG("osd: region %d attached to VENC %d at %d,%d layer %u alpha fg/bg %u/%u", handle,
                r->grp, r->x, r->y, hisi_osd_clamp_layer(r->layer), r->fg_alpha, r->bg_alpha);
 ...
 *
 * ss_mpi_rgn_set_chn_display_attr covers everything about a region except its
...
static int hisi_osd_push_chn_attr(hisi_state_t *st, int handle, hisi_osd_region_t *r)
{
    ...
    if (ret) {
        HAL_LOG_WARN("ss_mpi_rgn_set_chn_display_attr(region %d, VENC %d) failed: 0x%x", handle,
                     r->grp, ret);
        return RSS_ERR_IO;
    }
 ...
 * per-VENC-channel one (OT_RGN_VENC_MAX_OVERLAY_NUM), so it is counted per group
 ...
 * Mandatory rather than tidy: OT_ERR_RGN_BUSY is documented as "destroy a
...
int hal_osd_create_region(void *ctx, int *handle, const rss_osd_region_t *attr)
{
    ...
    if (attr->type == RSS_OSD_COVER) {
        ...
        HAL_LOG_INFO("osd: COVER regions are not composited by VENC on gen5; "
...
int hal_osd_destroy_region(void *ctx, int handle)
{
    ...
    if (ret)
        HAL_LOG_WARN("ss_mpi_rgn_destroy(%d) failed: 0x%x", handle, ret);
...
int hal_osd_register_region(void *ctx, int handle, int grp)
{
    ...
    if (hisi_osd_group_count(st, grp, handle) >= V5_RGN_OVERLAY_MAX_PER_VENC) {
        /* OT_RGN_VENC_MAX_OVERLAY_NUM. Refused here rather than left to the driver
         ...
         * it. */
        HAL_LOG_ERR("osd: VENC %d already carries %d overlays, the hardware maximum", grp,
                    V5_RGN_OVERLAY_MAX_PER_VENC);
        return RSS_ERR_NOMEM;
    }
 ...
 * at create time and ss_mpi_rgn_set_attr will not resize them, so it becomes
...
int hal_osd_update_region_data(void *ctx, int handle, const uint8_t *data)
{
    ...
    bmp.pixel_format = V5_PIXEL_FORMAT_ARGB_1555;
    bmp.width = r->width;
    bmp.height = r->height;
    bmp.data = r->bmp;
    ...
    if (ret) {
        HAL_LOG_WARN("ss_mpi_rgn_set_bmp(region %d, %ux%u) failed: 0x%x", handle, r->width,
                     r->height, ret);
        return RSS_ERR_IO;
    }
